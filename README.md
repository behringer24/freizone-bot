...
An unanswered invitation is not free, though: its facts arrived with it and stay on disk, so a bot whose address is known would grow one fact set per invitation for ever. They are dropped after 30 days (`FREIZONE_BOT_FORGET_INVITES_AFTER_HOURS`). Not sooner, because an invitation is announced exactly once — forgetting it early would close the door on inviting first and configuring the group afterwards, and inviting again would not reopen it, since from the group's side this bot is already invited.
...
| `FREIZONE_BOT_FORGET_INVITES_AFTER_HOURS` | `720` (30 days) | How long an **unanswered** invitation's facts are kept before they are dropped; `0` keeps them for ever. Ignoring an invitation does not remove what arrived with it, so a bot whose address is known grows one fact set per unsolicited invitation, without bound. Deliberately not immediate: an invitation is announced exactly *once*, so forgetting it early closes the door on "invite first, configure the group afterwards" — and inviting again does not reopen it, because from the group's side the bot is already invited. Never applies to the configured group, to a group the bot has joined, or to one it is not a member of. Every drop is logged with the group and the invitation's age. |
