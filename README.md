...
**Status:** `BOT-01`, `BOT-02`, `BOT-05`, `BOT-08`, `BOT-12` and most of `BOT-03` work, driven end to end against a real server rather than only in tests: the daemon registers its own account, joins the group it was configured for, delivers messages handed to it over a local socket or an optional HTTP ingress with a durable queue behind them, and answers commands from an allow-listed sender. Not there yet: pairing a `resolved` notice with its `firing` (`BOT-03`), a server-assistant role (`BOT-09`), interpretation by a model (`BOT-10`). See [`docs/ROADMAP.md`](docs/ROADMAP.md).
